...
const Navigation = ({ currentQuestion }) => {
  ...
  return (
    <div
      ...
      className="flex z-50 space-x-3 md:space-x-6 lg:space-x-8 xl:space-x-8 text-sm md:text-base lg:text-sm xl:text-base font-normal fixed bottom-0 left-0 right-0 justify-center lg:relative bg-[#1F2D45] lg:bg-transparent p-4 pt-2 pb-4 mr-5 lg:p-0"
    >
      <div
        id="question-one"
        className={`question 
      ...
      <div
        ...
        className={`question 
        ${
          currentQuestion + 1 === 6 && !isResultsPage
            ? "border-monzo-red"
            : "border-white/25"
        } 
        ${
          currentQuestion + 1 >= 6 && isResultsPage
            ? "border-monzo-results-green"
            : ""
        }
        2xl:w-24 text-center`}
      >
      ...
      >
        <p
          className={`transition duration-500 ease-in-out ${
            isResultsPage ? "text-white" : "text-white/25"
          } `}
        >
          Results
        </p>
