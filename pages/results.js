  ...
  return (
    <>
        ...
        <div id='content-container' className=' bg-monzo-black lg:pt-28 lg:pb-12 flex-1'>
          ...
          <div id='content-wrapper' className='relative flex flex-col lg:grid lg:grid-cols-12 lg:gap-x-8 xl:gap-x-12 justify-between max-w-[90%] lg:max-w-[94%] lg:w-[94%] 2xl:max-w-[1440px] mx-auto'>
          ...
          <motion.h1 
            ...
            viewport={{ once: true }} 
          className='w-full col-span-12 text-white text-center lg:text-left text-xl md:text-4xl lg:text-3xl xl:text-4xl 2xl:text-5xl font-bold leading-extra-tight lg:pt-8'>Here are your results</motion.h1>

          

          {/* TWO COL OF TITLE COPY  */}
          <div id='title-copy-wrapper' className='w-full col-span-12 text-white text-sm md:text-base'>
          ...
          <div id='chart-category-advice-container' className='col-span-12 bg-white'>

            {/* CHART AND SPEND CATEGORY SECTION */}
          
            <section id='chart-section' className='grid lg:grid-cols-12 gap-12 bg-monzo-off-white p-5 md:p-10 pb-10 md:pb-16'>

            <VisibilitySensor partialVisibility offset={{ bottom: 200 }} >
            {({ isVisible }) => (
              <div id='chart-left' className='lg:col-span-6 flex flex-col items-center md:space-y-4'>
                <div id='spend-title' className='flex flex-col items-center'>
                  <h2 className='uppercase font-bold text-xs md:text-base text-center'>your approximate average monthly spend</h2>
                  <h1 className='text-4xl md:text-5xl xl:text-7xl font-bold leading-snug'>£
                  {isVisible ? <CountUp end={rangeValues.reduce((prev, current) => {
                    return prev + current;
              ...
              <div id='chart-right' className='lg:col-span-6 space-y-6 2xl:space-y-8 flex flex-col justify-start'>

                {/* Duplicate this and make a component */}
                {questions[5].answers.map((spendingCategory, i) => {
                  let breakdownType = getBreakdownCategory(rangeValues[i], spendingCategory.min, spendingCategory.max);
                  return (
                    <SpendBreakdown
            ...
            <section id='advice-section' className='bg-monzo-off-white p-5 md:p-10 space-y-10'>
              ...
              <div id='title-wraper' className=' w-full flex justify-center'>
                <motion.h2 
                  ...
                  whileInView={{ opacity: 1, y: 0}} 
                  viewport={{ once: true }} 
                className='text-2xl md:text-4xl font-bold -mb-4'>Tips based on your answers</motion.h2>
              </div>

              <div id='advice-container' className='bg-white '>
                {questions.filter((quest) => { return parseInt(quest.number) < 6 }).map((question, qI) => {
                  return (
                    <div key={qI} className='advice p-5 md:p-10 border-b-1 border-grey-500 grid lg:grid-cols-12 gap-5 lg:gap-12'>
                      <div className='lg:col-span-6'>
                        <h3 className='text-base md:text-xl font-bold '><span className='text-monzo-results-orange'>Q{question.number}. </span>{question.text}</h3>
                        <p className='font-bold text-monzo-results-orange text-sm md:text-lg my-2 md:my-3'>Because you answered: -</p>
                      ...
                      <div className='lg:col-span-6 text-sm md:text-base'>
                        ...
                        {advice.filter((adv) => { 
                            ...
                            // display this advice.
                            return (parseInt(adv.questionOutput) === parseInt(question.number)) && 
                                   (arrayMatch(adv.possibleAnswers, selectedOptions).length > 0) 
                          }).map((adv, i) => {
                            // {adv.advice}
                            return (
                                <p key={i} className={`${i !== 0 ? 'my-3' : ''}`} dangerouslySetInnerHTML={{__html: adv.advice}}></p>
                            )
                        })}
                      </div>
                    </div>
            ...
            {/* COPY */}
            <section id='closing-words' className='p-5 md:p-10 bg-monzo-results-yellow'>
              <motion.h3 
                 initial={{ opacity: 0, y: 0 }} 
                 transition={{ ease: "easeInOut", duration: 0.5 }} 
                 whileInView={{ opacity: 1, y: 0}} 
                 viewport={{ once: true }} 
              className='text-2xl md:text-4xl font-bold mb-4'>Moving forward and finding help</motion.h3>
              <div className='grid lg:grid-cols-12 gap-4 lg:gap-12 text-sm md:text-base'>
                <div id='copy-wrapper-left' className='lg:col-span-6'>
                  <p className='text-lg md:text-xl font-medium '>Thanks for taking some time to reflect on your money habits and learn a little more about managing your money. We recommend taking a screenshot or printing this page to refer back to in future.</p>
                </div>
                <div id='copy-wrapper-right' className='lg:col-span-6'>
                  <p className='mb-4'>Your results will hopefully have given you a few ideas about how you can approach managing your money in the future. Remember, the results are a guide.</p>
                  <p>If you would like to find advice or seek support that takes your personal circumstances into consideration, you can <a target="_blank"  rel="noreferrer" className='tracking-normal underline text-monzo-red' href='https://monzo.com/help/monzo-flex/Managing-your-money'>find information within our money advice pages on the Monzo website</a>.</p>
                </div>
              </div>
            </section>
          </div>
